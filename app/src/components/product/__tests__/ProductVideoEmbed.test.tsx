...
describe('VideoEmbed — web iframe sandbox', () => {
  ...
  it('pins the sandbox allowlist and withholds the referrer', () => {
    mockPlatform('web');
    renderLoaded();
...
describe('VideoEmbed — native WebView', () => {
  it('whitelists only the nocookie origin, so an in-frame link cannot navigate away', () => {
    mockPlatform('ios');
    renderLoaded();
...
describe('VideoEmbed — non-YouTube URLs', () => {
  it('renders a plain link rather than framing an arbitrary origin', () => {
    mockPlatform('web');
    render(<VideoEmbed url="https://example.com/clip.mp4" linkColor="#000" />);
