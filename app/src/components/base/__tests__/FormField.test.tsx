...
describe('FormFieldError', () => {
  it('renders nothing when there is no message', async () => {
    await renderWithProviders(<FormFieldError errorId="field-error" message={undefined} />);
    expect(screen.queryByRole('alert')).toBeNull();
  });

  it('renders the message with a matching nativeID for describedBy linkage', async () => {
  ...
  it('announces the error explicitly on iOS, where VoiceOver ignores live regions', async () => {
    const announce = jest
    ...
    await rerender(<FormFieldError errorId="field-error" message="Required" />);
