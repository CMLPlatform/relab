...
export async function fetchWithTimeout(
  ...
  // Spreading a `Headers` instance or a tuple array would drop every header.
  const requestHeaders = new Headers(headers);

  // Correlation id on every request. A caller-supplied id (fetchWithAuth
  // reuses one across its 401 retry) is left intact.
