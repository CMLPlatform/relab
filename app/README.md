...
For the stack, source layout, routing, data flow, and testing layers, see
...
Three layers with different speed and confidence tradeoffs: `unit` and `integration` (both Jest;
`just test` runs both), and `e2e` (Playwright against a built web app; requires `just build-web` and
the `compose.e2e.yaml` stack). See [ARCHITECTURE.md](ARCHITECTURE.md#testing-layers) for the
location rules and lane selection.
...
through `AppThemeProvider` (see [ARCHITECTURE.md](ARCHITECTURE.md)).
...
React Native accessibility rules via `eslint-plugin-react-native-a11y`.
