...
from app.core.runtime import require_redis
...
async def _startup(app: FastAPI, services: AppServices) -> None:  # noqa: ARG001
    ...
    # Runtime init connects Redis (or aborts startup) before any domain starts.
    redis = require_redis(services.redis)
    set_email_checker(services, await init_email_checker(redis))
    set_common_password_checker(services, await init_common_password_checker(redis))
