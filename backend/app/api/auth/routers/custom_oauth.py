"""Custom OAuth router that handles redirecting logic to arbitrary frontend URLs."""
...
class BaseOAuthRouterBuilder:
    ...
    def _create_success_redirect(
        ...
        for raw_header in response.raw_headers:
            if raw_header[0] == SET_COOKIE_HEADER:
                redirect_response.headers.append("set-cookie", raw_header[1].decode("latin-1"))
        return redirect_response


class CustomOAuthRouterBuilder(BaseOAuthRouterBuilder, Generic[models.UOAP, models.ID]):  # noqa: UP046 # Excepted by fastapi-users
    """Builder for the main OAuth authentication router."""

    def __init__(
        self,
        oauth_client: BaseOAuth2,
        backend: AuthenticationBackend[models.UOAP, models.ID],
        get_user_manager: UserManagerDependency[models.UOAP, models.ID],
        state_secret: SecretType,
        redirect_url: str | None = None,
        cookie_settings: OAuthCookieSettings | None = None,
        *,
    ...
    def build(self) -> APIRouter:  # noqa: C901
        ...
        if self.redirect_url is not None:
            ...
            oauth2_authorize_callback = OAuth2AuthorizeCallback(self.oauth_client, route_name=self.callback_route_name)
        ...
        @router.get(
            "/callback",
            name=self.callback_route_name,
            description="The response varies based on the authentication backend used.",
        )
        async def callback(
            request: Request,
            access_token_state: Annotated[tuple[OAuth2Token, str], Depends(oauth2_authorize_callback)],
            user_manager: Annotated[BaseUserManager[models.UOAP, models.ID], Depends(self.get_user_manager)],
            strategy: Annotated[Strategy[models.UOAP, models.ID], Depends(self.backend.get_strategy)],
        ) -> Response:
                ...
                try:
                    if hasattr(response, "body"):
                        ...
                        if ACCESS_TOKEN_KEY in body:
                            access_token_str = body[ACCESS_TOKEN_KEY]
...
class CustomOAuthAssociateRouterBuilder(BaseOAuthRouterBuilder, Generic[models.UOAP, models.ID]):  # noqa: UP046 # Excepted by fastapi-users
    """Builder for the OAuth association router."""

    def __init__(
        self,
        oauth_client: BaseOAuth2,
        authenticator: Authenticator[models.UOAP, models.ID],
        get_user_manager: UserManagerDependency[models.UOAP, models.ID],
    ...
    def build(self) -> APIRouter:
        ...
        async def callback(
            request: Request,
            user: Annotated[models.UOAP, Depends(get_current_active_user)],
            access_token_state: Annotated[tuple[OAuth2Token, str], Depends(oauth2_authorize_callback)],
            user_manager: Annotated[BaseUserManager[models.UOAP, models.ID], Depends(self.get_user_manager)],
        ) -> Any:  # noqa: ANN401
            token, state = access_token_state
            ...
            frontend_redirect = state_data.get("frontend_redirect_uri")
            if frontend_redirect:
                placeholder_response = FastAPIResponse()  # Needed for helper
                return self._create_success_redirect(frontend_redirect, placeholder_response)
