...
BACKUP_REMOTE_HOST="${BACKUP_REMOTE_HOST?BACKUP_REMOTE_HOST not set}"
BACKUP_REMOTE_DIR="${BACKUP_REMOTE_DIR?BACKUP_REMOTE_DIR not set}"

# Safety Check: If the local dir has 0 files AND the remote has more than 0 files, abort.
if [ "$(find "$BACKUP_DIR" -type f | wc -l)" -eq 0 ] && \
   [ "$(ssh "$BACKUP_REMOTE_HOST" "find '$BACKUP_REMOTE_DIR' -type f 2>/dev/null | wc -l")" -gt 0 ]; then
    echo "[$(date)] ERROR: Local backup directory is empty, but remote is not. Aborting sync to prevent data loss."
    exit 1
fi

BACKUP_REMOTE="$BACKUP_REMOTE_HOST:$BACKUP_REMOTE_DIR"
