...
# Anchor outputs to the repo root, not the CWD, so --check works from anywhere.
...
def _expected_schemas() -> dict[Path, str]:
    ...
    return {
        ...
        # Consumed by the app's TypeScript codegen. Compact (large-file limit) and in
        # route order so api.generated.ts stays stable.
