...
"""Use Paracelsus to generate ERDs from the SQLAlchemy data schema."""
...
def main() -> None:
    """Entry point for the render ERD script."""
    # Get settings from pyproject.toml
    settings = get_pyproject_settings(config_file=CONFIG_FILE)

    # Generate ERD content
    logger.info("Generating complete ERD...")
    complete_erd = get_graph_string(
        base_class_path=settings.base,
        import_module=settings.imports,
        include_tables=set(),
        exclude_tables=set(),
        python_dir=[],
        format="mermaid",
