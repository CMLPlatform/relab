...
class TestPaginationEnvelope:
    ...
    async def test_response_contains_pagination_metadata(
        ...
        await MaterialFactory.create_async(session=session)
        response = await async_client.get("/materials")
        body = response.json()
        for key in ("total", "page", "size", "pages"):
            assert key in body, f"Missing pagination key: '{key}'"

    async def test_total_reflects_item_count(self, async_client: AsyncClient, session: AsyncSession) -> None:
        """Total must equal the number of materials in the database."""
        for i in range(3):
            await MaterialFactory.create_async(session=session, name=f"PaginationMaterial{i}")
        response = await async_client.get("/materials")
        body = response.json()
        assert body["total"] == len(body["items"])

    async def test_size_limits_returned_items(self, async_client: AsyncClient, session: AsyncSession) -> None:
        """?size=1 must return exactly one item regardless of total."""
        for i in range(3):
            await MaterialFactory.create_async(session=session, name=f"SizeMaterial{i}")
        response = await async_client.get("/materials?size=1")
        body = response.json()
    ...
    async def test_pages_metadata_matches_size_and_total(
        ...
        for i in range(3):
            await MaterialFactory.create_async(session=session, name=f"PageMeta{i}")
        response = await async_client.get("/materials?size=2")
        body = response.json()
        assert body["total"] >= 3
        assert body["size"] == 2
        assert body["pages"] >= 2
    ...
    async def test_page_beyond_total_returns_empty_items(
        ...
        await MaterialFactory.create_async(session=session)
        response = await async_client.get("/materials?size=1&page=9999")
        body = response.json()
        assert body["items"] == []
...
class TestPaginationSmoke:
    ...
    async def test_endpoint_returns_page_envelope(
        ...
        await factory.create_async(session=session)
        response = await async_client.get(path)
        assert response.status_code == status.HTTP_200_OK
        assert "items" in response.json()

    async def test_organizations_returns_page_envelope(self, async_client: AsyncClient, session: AsyncSession) -> None:
        """GET /organizations must return a Page envelope."""
        owner = await UserFactory.create_async(session=session)
        await OrganizationFactory.create_async(session=session, owner_id=owner.id)
        response = await async_client.get("/organizations")
        assert response.status_code == status.HTTP_200_OK
        assert "items" in response.json()

    async def test_admin_users_returns_page_envelope(self, superuser_client: AsyncClient) -> None:
