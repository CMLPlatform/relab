...
async def test_no_header_passes_through_without_touching_redis(redis_client: Redis) -> None:
    """A missing Idempotency-Key must be a complete no-op."""
    user_id = uuid4()

    async with idempotent_request(redis_client, user_id=user_id, endpoint=ENDPOINT, key=None, body=_Body()) as idem:
...
async def test_first_use_claims_and_stores_response_for_an_hour(redis_client: Redis) -> None:
    ...
    async with idempotent_request(redis_client, user_id=user_id, endpoint=ENDPOINT, key=key, body=_Body()) as idem:
        assert idem.replay is None
    await idem.finish(201, {"id": 42})

    ttl = await redis_client.ttl(f"idempotency:{user_id}:{ENDPOINT}:{key}")
    assert 60 < ttl <= 60 * 60


async def test_in_flight_marker_uses_a_short_ttl_not_the_response_ttl(redis_client: Redis) -> None:
    """The in-flight marker must self-heal quickly, not lock the key for the response TTL."""
    user_id = uuid4()
    key = "marker-ttl-key"

    async with idempotent_request(redis_client, user_id=user_id, endpoint=ENDPOINT, key=key, body=_Body()):
        ttl = await redis_client.ttl(f"idempotency:{user_id}:{ENDPOINT}:{key}")

    assert 0 < ttl <= 60


async def test_guard_releases_marker_when_create_raises(redis_client: Redis) -> None:
    ...
    async with idempotent_request(redis_client, user_id=user_id, endpoint=ENDPOINT, key=key, body=_Body()) as retry:
        assert retry.replay is None


async def test_failure_after_the_block_keeps_the_marker(redis_client: Redis) -> None:
...
async def test_replay_returns_stored_response_without_reprocessing(redis_client: Redis) -> None:
...
async def test_replay_with_a_different_body_is_rejected(redis_client: Redis) -> None:
    """Reusing a key for a different payload must 422, not hand back the earlier record."""
    user_id = uuid4()
    key = "reused-key"

    async with idempotent_request(
        redis_client, user_id=user_id, endpoint=ENDPOINT, key=key, body=_Body(name="a")
    ) as idem:
        pass
    await idem.finish(201, {"id": 1})

    with pytest.raises(HTTPException) as exc_info:
        async with idempotent_request(redis_client, user_id=user_id, endpoint=ENDPOINT, key=key, body=_Body(name="b")):
            pass
...
async def test_same_key_on_a_different_parent_is_a_separate_request(redis_client: Redis) -> None:
    ...
    async with idempotent_request(
        redis_client, user_id=user_id, endpoint="POST /products/1/components", key=key, body=body
    ) as idem:
        pass
    await idem.finish(201, {"id": 1})

    async with idempotent_request(
        redis_client, user_id=user_id, endpoint="POST /products/2/components", key=key, body=body
    ) as other:
        assert other.replay is None


async def test_cross_user_isolation_same_key_different_users(redis_client: Redis) -> None:
    """The same raw key for two different users must never collide."""
    user_a = uuid4()
    user_b = uuid4()
    key = "shared-raw-key"

    async with idempotent_request(redis_client, user_id=user_a, endpoint=ENDPOINT, key=key, body=_Body()) as idem:
        pass
    await idem.finish(201, {"id": 1})

    async with idempotent_request(redis_client, user_id=user_b, endpoint=ENDPOINT, key=key, body=_Body()) as for_b:
        assert for_b.replay is None


async def test_concurrent_in_flight_duplicate_returns_409(redis_client: Redis) -> None:
...
async def test_a_claim_that_vanishes_mid_flight_still_reports_the_request_as_running(redis_client: Redis) -> None:
...
async def test_unreachable_redis_returns_503_not_409(redis_client: Redis) -> None:
    ...
    with (
        ...
        async with idempotent_request(redis_client, user_id=user_id, endpoint=ENDPOINT, key="down-key", body=_Body()):
            pass
