...
from app.api.reference_data.schemas import CategoryCreate, MaterialCreateWithCategories, TaxonomyCreateWithCategories


def test_taxonomy_name_min_length() -> None:
    """Taxonomy name must be at least 2 characters."""
    with pytest.raises(ValidationError) as exc_info:
        TaxonomyCreateWithCategories(name="A", version="1.0", domains={TaxonomyDomain.MATERIALS})
...
def test_material_negative_density_rejected() -> None:
    ...
    with pytest.raises(ValidationError) as exc_info:
        MaterialCreateWithCategories(name="Invalid alloy", density_kg_m3=-100.0)
