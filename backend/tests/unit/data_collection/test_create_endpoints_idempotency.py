...
class _StubBody(BaseModel):
    ...
    Building a real ``ProductCreateWithComponents``/``ComponentCreateWithComponents`` would
    need a full bill-of-materials or nested components to pass validation, none of which this
    idempotency-wiring test exercises.
    """
...
async def test_create_product_replay_returns_stored_body_without_second_create(redis_client: Redis) -> None:
    ...
    with (
        ...
        first = await create_product(_StubBody(), current_user, session, redis_client, idempotency_key=key)  # ty: ignore[invalid-argument-type]
        second = await create_product(_StubBody(), current_user, session, redis_client, idempotency_key=key)  # ty: ignore[invalid-argument-type]
...
async def test_create_product_no_header_never_touches_redis(redis_client: Redis) -> None:
    ...
    with (
        ...
        await create_product(_StubBody(), current_user, session, redis_client, idempotency_key=None)  # ty: ignore[invalid-argument-type]
        await create_product(_StubBody(), current_user, session, redis_client, idempotency_key=None)  # ty: ignore[invalid-argument-type]
...
async def test_add_component_to_product_replay_avoids_second_create(redis_client: Redis) -> None:
    ...
    with (
        ...
        first = await add_component_to_product(db_product, _StubBody(), session, current_user, redis_client, key)  # ty: ignore[invalid-argument-type]
        second = await add_component_to_product(db_product, _StubBody(), session, current_user, redis_client, key)  # ty: ignore[invalid-argument-type]
...
async def test_add_component_to_component_replay_avoids_second_create(redis_client: Redis) -> None:
    ...
    with (
        ...
        first = await add_component_to_component(db_component, _StubBody(), session, current_user, redis_client, key)  # ty: ignore[invalid-argument-type]
        second = await add_component_to_component(db_component, _StubBody(), session, current_user, redis_client, key)  # ty: ignore[invalid-argument-type]
...
async def test_different_users_same_key_both_create(redis_client: Redis) -> None:
    ...
    with (
        ...
        await create_product(_StubBody(), user_a, session, redis_client, idempotency_key=key)  # ty: ignore[invalid-argument-type]
        await create_product(_StubBody(), user_b, session, redis_client, idempotency_key=key)  # ty: ignore[invalid-argument-type]
...
async def test_failure_after_the_commit_does_not_make_the_key_retryable(redis_client: Redis) -> None:
    ...
    with (
        ...
        with pytest.raises(RuntimeError, match="refresh blew up"):
            await create_product(_StubBody(), current_user, session, redis_client, idempotency_key=key)  # ty: ignore[invalid-argument-type]
