...
def test_generate_state_token_uses_configured_default_ttl(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("app.api.auth.services.oauth.utils.auth_settings.oauth_state_token_ttl_seconds", 600)
    monkeypatch.setattr("app.api.auth.services.oauth.utils.generate_jwt", fake_generate_jwt)
