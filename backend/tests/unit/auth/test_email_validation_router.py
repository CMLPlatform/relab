...
def _client() -> httpx.AsyncClient:
    ...
    return httpx.AsyncClient(transport=ASGITransport(app=app), base_url="https://test")
...
async def test_validate_email_rejects_invalid_body() -> None:
    """A non-email payload must 422, not reach the email checker."""
    async with _client() as client:
        response = await client.post("/v1/auth/validate-email", json={"email": "not-an-email"})
    assert response.status_code == 422


async def test_validate_email_rejects_missing_body() -> None:
    """A missing body must 422 rather than 500."""
    async with _client() as client:
        response = await client.post("/v1/auth/validate-email")
    assert response.status_code == 422


async def test_validate_email_accepts_valid_body() -> None:
    """A well-formed email passes validation (no disposable-email checker configured)."""
    async with _client() as client:
        response = await client.post("/v1/auth/validate-email", json={"email": "user@example.com"})
