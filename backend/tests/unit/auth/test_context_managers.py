...
@pytest.mark.parametrize("provided_session", [None, "provided"])
async def test_get_chained_async_user_manager_context(provided_session: str | None) -> None:
    """A provided session is used directly; otherwise a new session is created."""
    mock_session = AsyncMock() if provided_session else None
