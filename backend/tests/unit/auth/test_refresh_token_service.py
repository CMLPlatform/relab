...
class TestRefreshTokenService:
    ...
    async def test_create_refresh_token(self, redis_client: Redis) -> None:
        ...
        token = await create_refresh_token(redis_client, user_id, session_id)
        ...
        stored_data = await redis_client.get(f"refresh_token:{token}")
        assert stored_data is not None
        assert str(user_id) in stored_data
        assert session_id in stored_data

        # Verify token is in user's token set
        user_tokens = await redis_client.smembers(f"user_refresh_tokens:{user_id}")  # type: ignore[invalid-await] # redis-py stubs incorrectly include synchronous return types in the async client
        assert token in user_tokens

        # Verify TTL is set correctly (approximately 30 days)
        ttl = await redis_client.ttl(f"refresh_token:{token}")
    ...
    async def test_blacklist_token(self, redis_client: Redis) -> None:
        ...
        # Token should be removed from user's token set
        user_tokens = await redis_client.smembers(f"user_refresh_tokens:{user_id}")  # type: ignore[invalid-await] # redis-py stubs incorrectly include synchronous return types in the async client
    ...
    async def test_rotate_refresh_token(self, redis_client: Redis) -> None:
        ...
        old_token = await create_refresh_token(redis_client, user_id, session_id)
        ...
        is_blacklisted = await redis_client.exists(f"blacklist:{old_token}")
        assert is_blacklisted
        ...
        # User should have only the new token in their set
        user_tokens = await redis_client.smembers(f"user_refresh_tokens:{user_id}")  # type: ignore[invalid-await] # redis-py stubs incorrectly include synchronous return types in the async client
    ...
    async def test_multiple_tokens_per_user(self, redis_client: Redis) -> None:
        ...
        token_1 = await create_refresh_token(redis_client, user_id, session_1)
        token_2 = await create_refresh_token(redis_client, user_id, session_2)

        # Both tokens should be valid
        result_1 = await verify_refresh_token(redis_client, token_1)
        result_2 = await verify_refresh_token(redis_client, token_2)

        assert result_1["session_id"] == session_1
        assert result_2["session_id"] == session_2

        # User should have both tokens
        user_tokens = await redis_client.smembers(f"user_refresh_tokens:{user_id}")  # type: ignore[invalid-await] # redis-py stubs incorrectly include synchronous return types in the async client
        assert len(user_tokens) == 2
        assert token_1 in user_tokens
        assert token_2 in user_tokens
    ...
    async def test_token_expiry_ttl(self, redis_client: Redis) -> None:
        ...
        token_ttl = await redis_client.ttl(f"refresh_token:{token}")
        expected_ttl = settings.refresh_token_expire_days * 24 * 60 * 60

        # TTL should be close to expected (within 5 seconds)
        assert abs(token_ttl - expected_ttl) < TTL_ABS_MARGIN

        # User token set should have the same TTL
        user_set_ttl = await redis_client.ttl(f"user_refresh_tokens:{user_id}")
        assert abs(user_set_ttl - expected_ttl) < TTL_ABS_MARGIN
