...
@pytest.mark.parametrize("schema_cls", [UserRegister, UserUpdate])
def test_username_is_stripped_and_lowercased(schema_cls: type[UserRegister | UserUpdate]) -> None:
...
def test_user_create_rejects_non_slug_usernames(username: str) -> None:
    ...
    with pytest.raises(ValidationError):
        UserRegister(email="test@example.com", password=VALID_PASSWORD, username=username)
...
def test_reserved_usernames_are_rejected_after_normalization() -> None:
    ...
    with pytest.raises(ValidationError, match="reserved username"):
        UserRegister(email="test@example.com", password=VALID_PASSWORD, username="  Admin  ")
