...
class TestCameraStreamRouters:
    ...
    async def test_start_recording(
        ...
        # No prior recording session cached — idempotency check must fall through to new creation.
    ...
    async def test_stop_recording_ends_youtube_when_camera_offline(
        ...
        """A Pi outage must not prevent the YouTube broadcast from being torn down.

        Regression guard: previously the camera DELETE was called before end_livestream,
        so an offline Pi would raise and leave the YouTube broadcast orphaned on the
        user's channel.
        """
        mock_get_cam.return_value = mock_camera
        redis_mock = AsyncMock()
        ...
        # Simulate camera offline: DELETE raises a 503 ServiceUnavailableError.
        call_order: list[str] = []

        offline_error_msg = "camera offline"

        async def _camera_request(**kwargs: object) -> Response:
            call_order.append(str(kwargs.get("endpoint")))
            raise ServiceUnavailableError(offline_error_msg)
        ...
        # YouTube cleanup must precede the camera cleanup attempt.
        assert call_order[0] == "youtube_end", f"YouTube must be ended first, got {call_order}"
        assert call_order[1] == PLUGIN_STREAM_ENDPOINT
        mock_yt_service.end_livestream.assert_awaited_once_with(FAKE_BROADCAST_KEY)
        # Video still created, session still cleared even though the Pi was offline.
        mock_create_video.assert_awaited_once()
        assert result.product_id == 1
        redis_mock.delete.assert_awaited_once_with(f"rpi_cam:youtube_recording:{camera_id}")
    ...
    async def test_start_recording_is_idempotent_when_session_already_active(
        ...
        """A retry of start_recording must return the live StreamView, not create a second broadcast."""
        del mock_db_check
        mock_get_cam.return_value = mock_camera
        ...
        # Redis returns an existing session for this camera.
        redis_mock = AsyncMock()
        redis_mock.get.return_value = json.dumps(
            {
                "product_id": 1,
        ...
        # The live StreamView is returned from the verification GET, not from a fresh POST.
        assert str(result.url) == f"{YOUTUBE_STREAM_URL}/"
        # No new broadcast created — setup_livestream must never run.
        mock_yt_service.setup_livestream.assert_not_called()
        # Exactly one camera call: the GET /stream verification. No POST/DELETE.
        mock_camera_request.assert_awaited_once()
