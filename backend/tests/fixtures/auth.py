...
async def db_user(db_session: AsyncSession) -> User:
    ...
    return await UserFactory.create_async(session=db_session, is_superuser=False, is_active=True)


@pytest.fixture
async def db_superuser(db_session: AsyncSession) -> User:
    """Create a superuser for admin and DB-backed tests."""
    return await UserFactory.create_async(session=db_session, is_superuser=True, is_active=True)
