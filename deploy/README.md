...
Three runbooks on two axes: which host, and whether it is a routine release or the
one-time migration off `main`. Staging's cutover is done, so only prod still has one.
...
migration, the data checks, the encryption pass.
**Delete `CUTOVER-PROD.md` once the MVP migration is done on prod**, as `CUTOVER-STAGING.md` already
was. They reference `DEPLOY-*` rather than duplicating it, so nothing durable is lost when they go.
At deletion time, also drop their cell from the table above and grep for `CUTOVER-` — a couple of
code comments name the migration steps they describe (`backend/justfile`'s `backfill-upload-sizes`
goes with them).
