...
export default function ProductCamera() {
  ...
  return (
    <View style={{ flex: 1 }} onLayout={() => setReady(true)}>
      ...
      {ready && <CameraView ref={ref} facing="back" style={{ flex: 1 }} onCameraReady={() => setCameraReady(true)} />}

      {/*Only enable taking pictures when camera is actually ready*/}
      {cameraReady && <CameraButton onPress={takePicture} />}
    </View>
  );
}
...
function CameraButton({ onPress }: { onPress: () => void }) {
  // Render
  return (
    <Pressable onPress={onPress} style={{ position: 'absolute', bottom: 30, alignSelf: 'center' }}>
      {({ pressed }) => (
        <View
          style={{
