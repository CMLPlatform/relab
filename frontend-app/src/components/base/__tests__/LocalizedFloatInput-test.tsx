...
describe('LocalizedFloatInput', () => {
  ...
  it('calls onChange with undefined on blur when input is empty', () => {
  ...
  it('calls onChange with parsed number on valid blur', () => {
    const onChange = jest.fn();
  ...
  it('reverts to previous value when entered value is below min', () => {
    ...
    // "3" passes the decimal regex but fails the min=5 check
  ...
  it('ignores non-numeric characters during text change', () => {
  ...
  it('accepts valid decimal text during typing', () => {
    render(<LocalizedFloatInput value={undefined} />);
    const input = screen.getByPlaceholderText('> 0');
