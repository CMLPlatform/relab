...
For the authoritative schema, request models, and full endpoint list, use the [interactive API documentation](https://api.cml-relab.org/docs). For how the API is designed internally, see [API Structure](../architecture/api.md).
...
- Refresh-token handling depends on the Redis-backed auth path (see [Authentication](../architecture/auth.md))
...
- If you need a stable exported dataset rather than live application access, check the [Dataset Documentation](../dataset/index.md) first.
