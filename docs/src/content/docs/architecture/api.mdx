...
Routes are organized by domain, not by HTTP verb. The same API serves the research app, the public web frontend, and external tooling. Public and authenticated surfaces are separated in practice, but the cleanest source of truth is the live OpenAPI docs.

For live schemas and request models, start from the [API reference overview](/api-reference/). For practical usage guidance, see the [API interaction guide](../../user-guides/api/).

There are three distinct surfaces: the public app API is the contract for the
frontend and external clients, the device API handles Pi-originated HTTP
callbacks, and a small private DTO seam covers backend device integration plus
the RPi camera plugin.
...
Product and component entry is progressive. A record can be created before its full component tree or bill of materials is known; completeness checks belong to later curation and audit workflows, not to create-time validation.

Product- and component-create requests (`POST /v1/products`, `POST /v1/products/{id}/components`, `POST /v1/components/{id}/components`) accept an optional `Idempotency-Key` header so a client can safely retry a request whose response was lost after the server had already committed it. The first request with a given key is processed normally; a retry with the same key returns the original response instead of creating a duplicate record.

A key is only replayed when everything about the retry matches the original:

- the same authenticated user, so a key cannot be replayed across accounts
...
- the same request body — reusing a key with a different payload returns `422`, rather than silently returning the earlier record

A retry that arrives while the first request is still running returns `409`. If the idempotency store is unreachable the request fails closed with `503` instead of risking a duplicate create. Stored responses expire after one hour.
...
- Public reference data and public research reads are openly accessible; user-owned workflows require authentication, and mutations are limited to verified owners or superusers by backend dependencies.
