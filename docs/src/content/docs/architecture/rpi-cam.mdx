...
graph TD
    ...
    MainAPI -->|WebSocket relay| RpiCamAPI["fa:fa-microchip RPI Camera API"]
    FrontendApp -.->|LAN direct| RpiCamAPI
    RpiCamAPI -->|Controls| Camera["fa:fa-camera Camera Hardware"]

    RpiCamAPI -->|HLS preview| FrontendApp
    RpiCamAPI -.->|Stream egress| YouTube["fa:fa-video YouTube API"]
    RpiCamAPI -->|Image upload| MainAPI
...
- **Device WebSocket protocol**: `/v1/plugins/rpi-cam/ws/connect` for the persistent outbound relay tunnel from the Pi to the backend.
