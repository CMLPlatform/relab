...
- Pairing, relay orchestration, capture storage, and YouTube coordination stay in the main backend, which acts as the control plane.
- There are two contract layers: a public app-facing API plus a smaller private device contract for pairing, relay, and uploads (see [Protocol boundaries](#protocol-boundaries)).
- After pairing, the app can switch to LAN-direct access with a device-local API key, for lower-latency preview and capture when the device is reachable locally.
- Cameras pair through a short-lived code and receive runtime relay credentials from the backend. Operators do not manually copy long-lived API keys in the normal path.
- Captured images are stored in Relab's file storage and linked to the originating product or component record automatically.
- YouTube streaming is optional and mediated through the backend's OAuth connection to YouTube. The device streams directly to YouTube once authorized.
