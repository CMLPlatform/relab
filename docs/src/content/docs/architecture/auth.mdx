...
Relab supports bearer tokens for API clients and httpOnly cookies for browser sessions. Redis backs auth token storage, MFA challenge state, and rate limiting.
...
Access tokens are short-lived: the application access-token lifetime is 15 minutes. Auth flows require Redis and fail closed when Redis-backed token state is unavailable.

Successful login updates `last_login_at` without storing full login IP addresses on user accounts. Client IP addresses may still be processed transiently for Redis-backed rate limiting. Login also creates a refresh token. Browser sessions receive refresh and access cookies; bearer clients receive tokens in the response body.
...
Refresh tokens are random bearer secrets. Redis stores them under SHA-256 fingerprints rather than raw token values, so storage keys are not usable credentials if exposed. OAuth provider tokens that must remain reversible, such as Google/YouTube access and refresh tokens, are encrypted before database storage with AES-256-GCM using the backend `DATA_ENCRYPTION_KEY`.

Encryption does not replace authorization: authenticated API handlers check resource ownership and route-level permissions before returning or mutating data.
...
Frontend OAuth callback results are carried only in the URL fragment: `#status=success`, `#status=error&error=<code>`, or `#status=mfa_required&mfa_handoff=<one-time-handoff>`. The app handles the fragment, clears it from browser history, and never receives Relab access tokens through callback URLs. When OAuth login needs MFA, the app claims the one-time handoff through the API, stores the returned pending MFA challenge in app runtime/session storage, and routes immediately to the MFA screen. The MFA challenge token itself is never placed in query strings or URL fragments.
