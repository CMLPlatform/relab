...
Access tokens carry their issue time, and each revocation records a per-user epoch in Redis; a token issued before that epoch is refused on its next request. Revocation is therefore immediate rather than bounded by the 15-minute token lifetime, and costs one write regardless of how many sessions the account has. Refresh state is always purged before the epoch is stamped, so a refresh racing the revocation cannot mint a token that outlives it.
