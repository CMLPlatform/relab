...
TOTP MFA is optional account hardening. Accounts without TOTP enabled receive bearer tokens or browser session cookies after successful password or OAuth login. Accounts with TOTP enabled receive a short-lived challenge token and must submit a current one-time code before the API issues bearer tokens or browser session cookies. TOTP setup is an authenticated account action, and future sensitive routes can use the backend MFA dependency to require accounts with confirmed MFA. TOTP seeds and pending MFA tokens are generated with cryptographically secure randomness, and accepted TOTP counters are recorded briefly to prevent replay within the allowed clock window. Administrators can reset a user's MFA enrollment after identity-proofed recovery, but they cannot choose a replacement factor for the user.
...
Refresh tokens have a 30-day inactivity lifetime and a 30-day absolute session lifetime. Each successful refresh rotates the refresh token, but rotation preserves the original absolute expiry, so activity cannot extend a session beyond 30 days. RELab allows multiple concurrent sessions per account within that lifetime. Users can end all of them from the profile screen with "Sign out everywhere"; the current product direction intentionally does not add a per-device session inventory.

Bearer logout revokes the submitted refresh token. Session logout clears auth cookies, asks the browser to clear local session data, and blacklists the refresh token when one is present. `POST /v1/auth/sessions/revoke-all` revokes all refresh tokens for the current account and clears browser session state. Password reset, current-password-protected password changes, current-password-protected email changes, account deactivation, and account deletion also revoke all refresh tokens for the affected account. Existing access tokens are not tracked in a separate revocation list; authenticated API requests still require a resolvable active user, and any residual access-token exposure is bounded by the 15-minute lifetime.
...
OAuth login is available for Google and GitHub. Both providers support session-cookie callbacks, bearer-token callbacks, and account linking. Google accounts may be linked by verified email. GitHub linking requires explicit association. OAuth login creates RELab-owned access and refresh sessions; RELab does not currently claim IdP-wide logout or coordinated provider session termination.
...
The public backend origin comes from `API_PUBLIC_URL`; production currently uses `https://api.cml-relab.org`. Bearer callback routes use the same pattern, such as `/v1/oauth/google/token/callback`, when enabled for a client.
