...
The backend uses `DATA_ENCRYPTION_KEYS` only for reversible sensitive values it must recover later, currently OAuth provider tokens and active YouTube broadcast keys. Keep those keys in the host runtime environment, not in committed files.

For rotation, prepend a new key to `DATA_ENCRYPTION_KEYS`, deploy, run `just reencrypt-sensitive-fields` from `backend/`, and keep retired keys until old backups no longer need online decryption.
