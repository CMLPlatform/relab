...
The backend enforces source-controlled application limits: JSON bodies are counted while streaming, multipart uploads have per-media size caps and allowlisted formats, expensive public product suggestion/facet routes, authenticated product/component writes, and upload routes use Redis-backed rate limits, and the RPi camera WebSocket relay bounds auth attempts and frame sizes.
...
The Expo web app still requires inline and eval-style first-party script support in its enforced CSP. RELab keeps a stricter report-only policy as the migration target; this is the remaining ASVS V3 L3 CSP exception for the app origin. RELab does not currently collect CSP violation reports because there is no reviewed reporting pipeline yet. The docs and public web origins enforce stricter script policies today, while all browser-facing origins enforce `object-src 'none'`, `base-uri 'none'`, `frame-ancestors 'none'`, and referrer suppression.
