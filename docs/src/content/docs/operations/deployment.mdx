...
ClamAV malware scanning is opt-in, not enabled by default. Turning it on for prod or staging takes two agreeing settings: `MALWARE_SCAN_ENABLED=true` in the root `.env`, and starting the stack with the `scanning` Compose profile (`just prod-up YES scanning` or `just staging-up YES scanning`). Leaving the flag `true` without the profile fails all uploads closed. The default, unscanned mode — `MALWARE_SCAN_ENABLED=false` and no `scanning` profile — is an explicit accepted risk: uploaded files are stored without a malware scan. When scanning is enabled, the `clamav` service keeps its signature database in the persisted `clamav_db` volume; first boot with an empty volume can wait several minutes while signatures download. Budget roughly 3-4 GiB of RAM for ClamAV so default signature reload and database testing behavior can stay enabled.
