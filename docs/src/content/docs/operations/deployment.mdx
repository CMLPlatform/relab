...
Prod and staging enable ClamAV malware scanning for uploads. The `clamav` service keeps its signature database in the persisted `clamav_db` volume; first boot with an empty volume can wait several minutes while signatures download. Budget roughly 3-4 GiB of RAM for ClamAV so default signature reload and database testing behavior can stay enabled.
