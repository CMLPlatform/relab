...
Backup services are enabled through Compose profiles. Telemetry is opt-in through host-level environment variables: `LOKI_PUSH_URL` enables the optional Loki logging overlay, and `OTEL_EXPORTER_OTLP_ENDPOINT` enables backend OTLP export.
...
- Optional offsite copies use `restic copy`; WebDAV is supported through restic's rclone backend. When `RESTIC_OFFSITE_REPOSITORY` is set in the root `.env`, the scheduled backup service copies snapshots offsite at the end of every backup cycle automatically. `just backup-offsite-copy <env>` remains available as an ad-hoc, on-demand copy outside that cycle.
...
- `LOKI_PUSH_URL` in the host root `.env` enables the optional Loki Docker log-driver overlay.
...
- A minimal watchdog exists: `just watchdog <env>` checks the API container's health and the age of the newest restic snapshot, exiting non-zero with an `ALERT[...]` line per failure. It has no built-in scheduler, so run it from host cron to make sure failures reach an operator. Richer alerting (Loki alert rules, external uptime monitoring, Grafana) is still future work.
