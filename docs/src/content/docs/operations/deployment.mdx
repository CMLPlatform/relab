...
- A backup refuses to archive a dump, or an uploads tree, smaller than half the newest stored snapshot of the same tag, because an empty one would otherwise become the newest snapshot and age out the good copies. `RESTIC_MIN_DUMP_RATIO` tunes it; `0` disables it.
...
- `just restore-check <env>` restores the latest database dump into a disposable Postgres container and verifies that it can be read, and restores the latest uploads snapshot into a scratch directory to verify it carries real data.
