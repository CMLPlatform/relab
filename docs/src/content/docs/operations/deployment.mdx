...
graph TD
    ...
    accDescr: Core services — the FastAPI backend, PostgreSQL, Redis, ClamAV, user media storage, the Expo app, the Astro landing page, and the Starlight docs site — run alongside optional operational services for migrations, backups, the Cloudflare tunnel, and Loki or OTLP telemetry export; the backend is the hub, talking to the database, cache, malware scanner, and external monitoring collector.
