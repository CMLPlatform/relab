...
- The same `compose.deploy.yaml` is used for prod and staging. Committed `deploy/env/*.compose.env` files select the environment; Docker Compose reads the host-level root `.env` for local interpolation values such as tunnel tokens, telemetry endpoints, and backup paths. Manual shell helpers use exported environment variables instead of parsing `.env`.
...
`just compose-policy-check` validates these rendered Compose invariants for prod and staging.
...
CI checks Dockerfile misconfigurations with Trivy and validates Cloudflare edge IaC with `just cloudflare-check`, while RELab-specific Compose policy checks validate rendered stack invariants such as network exposure, runtime hardening, image pinning, and secret access.
...
The backend uses `DATA_ENCRYPTION_KEY` only for reversible sensitive values it must recover later, such as OAuth provider tokens and active YouTube broadcast keys. Cache payload signing uses the separate `CACHE_SIGNING_SECRET`, and OAuth state uses `OAUTH_STATE_SECRET`, so each key has one purpose. Keep these application secrets in the host runtime environment, not in committed files.
...
configuration, `deploy/` contains committed runtime config and manifests,
`secrets/` contains uncommitted operator-owned secret material, and `scripts/`
contains repo-wide operational helpers that are too large for the root
`justfile`. Non-secret backend runtime settings stay in `backend/.env.<env>`
because they are consumed directly by the FastAPI settings layer and backend
tooling.
