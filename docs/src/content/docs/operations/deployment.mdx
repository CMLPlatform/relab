...
- Database dumps and user-uploaded files are backed up into an encrypted local restic repository under `BACKUP_HOST_DIR`. The backup container runs as UID/GID `1001`.
...
- Offsite copies use `restic copy` over restic's rclone backend, to the one remote defined in `secrets/<env>/rclone.conf`; `RESTIC_OFFSITE_REPOSITORY` in the root `.env` overrides that target.
...
- Alembic migrations move schema state forward and run in one transaction, so a failed migration leaves the previous revision in place. `just <env>-rollback YES <sha> [<revision>]` returns to the images a previous build tagged, and downgrades the schema only when no migration in the range destroyed data; otherwise restore a backup.
